...
cellule_t* trouver_queue_seq(sequence_t* seq){
    ...
    if (!cel->suivant || (verif_type(2, cel) && !cel->buffer.fin_groupe->suivant)){
        seq->tete = NULL;
        return cel;
    }

    while(cel->suivant->suivant){

        if (verif_type(2, cel)){
                
            cellule_t* apres_cel = cel->buffer.fin_groupe->suivant;

            if (verif_type(2, apres_cel) && !apres_cel->buffer.fin_groupe->suivant){
                cel->buffer.fin_groupe->suivant = NULL;
                return apres_cel;                    
            }
            else if (verif_type(1, apres_cel) && !apres_cel->suivant){
                cel->buffer.fin_groupe->suivant = NULL;
                return apres_cel;
            }
            else if (verif_type(0, apres_cel) && !apres_cel->suivant){
                cel->buffer.fin_groupe->suivant = NULL;
                return apres_cel;
            }
...
cellule_t* inversee_seq(sequence_t* seq){
    ...
    sequence_t seq_tmp;
    seq_tmp.tete = NULL;
    cellule_t cel_seq = seq_tmp.tete;
    while (seq->tete){
        cellule_t* queue = trouver_queue_seq(seq);
        cellule_t* clone_queue = clone_cellule(queue);

        if (!seq_tmp.tete){
            seq_tmp.tete = clone_queue;
            cel_seq = seq_tmp.tete;
        }
        else{
            cel_seq->suivant = clone_queue;
            cel_seq = clone_queue;
        }
